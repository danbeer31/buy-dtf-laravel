                        ...
                        <table class="table align-middle mb-0">
                            ...
                            <tbody>
                                @forelse($orders as $order)
                                    @php
                                        $eligibleImages = $order->dtfImages->filter(fn($img) => ($img->item_type ?? null) !== 'gang_sheet');
                                        $groupImageIds = $eligibleImages
                                            ...
                                            ->values();
                                        $forceReadd = $eligibleImages->isNotEmpty()
                                            && $eligibleImages->every(fn($img) => (int) $img->production === 1);
                                    @endphp
                                        ...
                                        <td class="py-3 text-end pe-4">
                                            <div class="btn-group">
                                                ...
                                                <button class="btn btn-sm btn-info fw-bold px-3 add-order-to-production"
                                                        data-id="{{ $order->id }}"
                                                        data-images="{{ json_encode($groupImageIds->toArray()) }}"
                                                        data-row-count="{{ $eligibleImages->count() }}"
                                                        data-readd="{{ $forceReadd ? '1' : '0' }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            async function processImage(imageId, index, total, options = {}) {
                ...
                progressStatus.innerText = `Processing group ${index + 1} of ${total}...`;
            ...
            document.querySelectorAll('.add-order-to-production').forEach(btn => {
                btn.addEventListener('click', async function() {
                    ...
                    const rowCount = parseInt(this.dataset.rowCount || imageIds.length, 10);
                    const readd = this.dataset.readd === '1';
                    ...
                    const action = readd ? 'Re-upload' : 'Add';
                    if (!confirm(`${action} ${imageIds.length} production groups from ${rowCount} image rows?`)) return;

                    progressModal.show();

                    let successCount = 0;
                    for (let i = 0; i < imageIds.length; i++) {
                        const success = await processImage(imageIds[i], i, imageIds.length, { readd });
